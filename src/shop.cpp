...
void Shop::buyAppearance(Player &player){
    ...
    for(int i = 0; i < availableSkins.size(); i++){
        if(std::find(ownedSkins.begin(), ownedSkins.end(), availableSkins[i]) == ownedSkins.end()){
            ...
            ui::printStr(std::to_string(i+1)+". "+availableSkins[i]+" ("+std::to_string(APPEARANCE_PRICE)+" coins)",
                         16+i+1, 1, "white");
        }
    }
...
//             }
        ...
        if(player.getWealth() >= APPEARANCE_PRICE){
            ...
            ui::printStr("Do you wish to equip the skin you just bought? (0 for YES, 1 for NO)",
                         16+availableSkins.size()+4, 1, "white");
            choice = getch();
            if(choice == '0'){
...
void ShoppingMap::show_map(Shop &p, Player &real_p){
    ...
    ui::printStr("Wealth: "+std::to_string(real_p.getWealth())+"\t\tLevel: "+std::to_string(real_p.getLevel()), 2,
                 width+5, "white");
